...
[Canny extension for End-users](browser-extension/README-ja.md) という連携するブラウザ拡張もあります。Canny の投稿一覧ページに直接介入し、各投稿の投稿者名や投稿日の表示、自分の投稿への印付け、投稿者による絞り込みなどを行います。本ウェブツールが「検索条件で開く Canny の URL を組み立てる」のに対して、拡張は「開いた先のページの見やすさを改善する」を担う、相補的な関係になっています。拡張はブラウザへのインストールが必要ですが、本ウェブツールはインストール不要です。


## 関連: 活用事例集

[COOKBOOK-ja.md](COOKBOOK-ja.md) は、VRChat Canny サイトでよくある場面で本ツールとブラウザ拡張がどう役立つかを示すレシピ集です。
